        ...
        <div className="mt-24 max-w-[900px]">
          ...
          <div className="bg-white rounded-xl border p-6 min-h-[300px]">
            ...
            {[{
              ...
              <div key={pedido.id} className="border rounded-xl p-5 mb-6 bg-white shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-lg font-bold text-indigo-950">Pedido #{pedido.id} – R${pedido.valor.toFixed(2)}</div>
                  <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">{pedido.status}</span>
                </div>
                <div className="text-sm text-zinc-700 mb-2">Comprado por {pedido.comprador} em {pedido.data}</div>
                <div className="text-xs text-zinc-500 mb-4">Tipo cobrança: {pedido.tipoCobranca}</div>
                <div className="border rounded-lg bg-zinc-50">
                  <div className="px-4 py-3 font-semibold text-indigo-900">Ingressos</div>
                    ...
                    <tbody>
                      {pedido.ingressos.map((ing, idx) => (
                        <tr key={idx} className="border-t">
                          <td className="px-4 py-2">{ing.tipo}</td>
                          <td className="px-4 py-2">R$ {ing.preco.toFixed(2)}</td>
                          <td className="px-4 py-2">{ing.participante}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
