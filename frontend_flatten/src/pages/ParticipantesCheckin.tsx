    ...
    <div className="min-h-screen w-full bg-white">
      ...
      <div style={{ marginLeft: totalLeft, transition: 'margin-left 200ms' }} className="flex flex-col pl-8 pr-8 min-h-screen relative">
        <div className="mt-24 max-w-[900px]">
          <h1 className="text-3xl font-bold text-indigo-950 mb-3">Check-in</h1>
          <p className="text-gray-700 mb-6">Faça check-in dos participantes usando seu nome</p>
          <div className="w-full h-2 bg-zinc-200 rounded mb-2 relative">
            <div className="h-2 bg-green-400 rounded" style={{ width: `${(checkedCount/total)*100}%` }}></div>
            <div className="absolute right-0 -top-6 text-sm font-semibold text-zinc-700">{checkedCount} / {total}</div>
          </div>
          <div className="flex items-center justify-between mb-4">
            <input type="text" placeholder="Digite o Nome do participante" className="border rounded-lg px-4 py-2 text-sm w-72" />
              ...
              <tbody>
                {participantes.map((p, idx) => (
                  <tr key={idx} className="border-t">
                    ...
                    <td className="px-6 py-3">{p.ingresso}</td>
                    <td className="px-6 py-3">
                      {checkedIn[idx] ? (
                        <span className="inline-flex items-center text-green-600 font-semibold gap-2">
                          ...
                          <button className="text-xs text-zinc-500 hover:underline ml-2" onClick={() => handleCancelCheckin(idx)}>
                            Cancelar
                          </button>
                        </span>
                      ) : (
                        <button className="text-indigo-700 font-semibold hover:underline" onClick={() => handleCheckin(idx)}>
                          Check-in
                        </button>
                      )}
                    </td>
