...
const OrganizationDropdown: React.FC = () => {
  const { orgs, selectedOrg, setSelectedOrgById, loading, error } = useOrganization();
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="text-[#091747] font-bold text-[15px] max-w-[140px] truncate" title={label}>{label}</span>
        ...
        <div ref={panelRef} className="absolute left-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-blue-200 z-50 flex flex-col max-h-[320px] overflow-auto">
          ...
          {Array.from(new Map(orgs.map(o=> [o.id, o])).values()).map(o => {
            ...
            return (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-[11px] font-bold text-indigo-700">{o.name.slice(0,2).toUpperCase()}</div>
          ...
          <button
            ...
            disabled={loading}
            onClick={() => { setOpen(false); /* TODO: modal criar org */ }}
          >
            + Criar organização
          </button>
        </div>
...
const AppHeader: React.FC = () => {
  ...
  return (
    <div style={dynamicStyle} className={`flex absolute top-0 left-0 w-full items-center pr-5 py-4 bg-white border-b border-solid border-zinc-100 h-[60px] max-md:relative max-md:flex-wrap max-md:gap-4 max-md:px-5 max-md:py-4 max-md:w-full max-sm:px-4 max-sm:py-2.5 transition-all duration-200`}>
      {/* Left: apenas seletor de organização */}
