...
const LocationSelector: React.FC = () => {
  const [selectedLocation, setSelectedLocation] = useState('Ceará');
  const [isOpen, setIsOpen] = useState(false);

  const locations = ['Ceará', 'São Paulo', 'Rio de Janeiro', 'Bahia', 'Minas Gerais'];

  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-[142px] h-[33px] border shadow-[0_4px_12.9px_0_rgba(0,0,0,0.05)] flex items-center bg-white px-4 py-0 rounded-[16.5px] border-solid border-[#E0E0E0] hover:border-[#2A2AD7] transition-colors"
      >
        <svg width="10" height="13" viewBox="0 0 10 13" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-3">
        ...
        <span className="text-[#091747] text-sm font-bold flex-1 text-left">{selectedLocation}</span>
