...
import { resolveEventLocationLabel } from '@/lib/eventLocation';
...
const dateKey = (event: any) => eventDateKey(event);
const dateLabel = (event: any) => {
  ...
  const dayMonth = dateKey(event) === todayKey
    ? 'Hoje'
    : new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, day: 'numeric', month: 'short' }).format(date);
...
const timeDetails = (event: any) => {
  ...
  if (!timezone || timezone === 'America/Sao_Paulo' || timezone === 'America/Fortaleza') return { primary, secondary: '' };
...
const eventLocation = (event: any) => resolveEventLocationLabel(event);

const organizerName = (event: any, organization: any) => {
  const fullName = event.registrationForm?.externalHost || event.organizerName || event.organizer?.name || event.organization?.name || event.createdByUser?.name || organization?.name || 'Organizador';
  return String(fullName).trim();
};

const StickyEventDate = ({ event }: { event: any }) => {
...
export default function CalendarPublicEventViews({
  ...
  const groups = React.useMemo(() => {
    ...
    events.forEach((event) => grouped.set(dateKey(event), [...(grouped.get(dateKey(event)) || []), event]));
  ...
  return (
    ...
    >
      {groups.map((group) => {
        return (
          <section className="cp-event-group" key={dateKey(group[0])}>
            <StickyEventDate event={group[0]} />
            <div className="cp-event-items">
              {group.map((event) => {
                const image = resolveImageUrl(event.bannerUrl || event.banner || event.image);
                const avatar = resolveImageUrl(event.organizerPhotoUrl || event.organizer?.photoUrl || event.organization?.logoUrl || organization?.logoUrl);
                ...
                if (external) return (
                  <article className={`cp-external-card ${variant === 'list' ? 'is-list' : ''}`} key={event.id} onClick={() => onEventClick(event)}>
                    <div className="cp-event-card-main">
                      ...
                      <div className="cp-event-cover">{image ? <EventImage event={event} alt="" /> : <Sparkles size={28} />}</div>
                    </div>
...
const styles = `
.cp-events,.cp-event-group,.cp-event-items{width:100%;min-width:0;max-width:100%;box-sizing:border-box}.cp-events{color:var(--cp-primary)}.cp-event-group{position:relative;margin-bottom:24px;padding-bottom:0}.cp-event-date-sentinel{display:block;width:1px;height:0;pointer-events:none}.cp-event-date{position:relative;z-index:20;margin-bottom:14px;text-transform:lowercase}.cp-event-date-content{position:relative;display:inline-flex;align-items:baseline;gap:6px;border:1px solid transparent;border-radius:100px;background:transparent;transition:border-color .16s ease,background-color .16s ease,box-shadow .16s ease}.cp-event-date strong{font-size:16px;font-weight:600;text-transform:none}.cp-event-date-content>span{color:var(--cp-secondary);font-size:16px;font-weight:500}.cp-event-items{display:grid;gap:16px}
.cp-events--cards .cp-event-group{padding-left:24px}.cp-events--cards .cp-event-group:before{content:'';position:absolute;left:4px;top:16px;bottom:-25px;border-left:2px dashed var(--cp-border)}.cp-events--cards .cp-event-date i{position:absolute;left:-20px;top:50%;width:8px;height:8px;transform:translateY(-50%);border-radius:50%;background:var(--cp-secondary);backdrop-filter:blur(16px)}
.cp-event-card{display:flex;width:100%;min-width:0;max-width:100%;box-sizing:border-box;overflow:hidden;flex-direction:column;align-items:flex-start;gap:10px;padding:12px 12px 12px 16px;border:1px solid var(--cp-border);border-radius:12px;background:var(--cp-card);box-shadow:none;cursor:pointer;transition:border-color .16s ease,box-shadow .16s ease}.cp-event-card:hover{border-color:var(--cp-hover-border);box-shadow:var(--cp-hover-shadow)}.cp-event-card-main{display:flex;width:100%;min-width:0;align-items:flex-start;justify-content:space-between;gap:16px}.cp-event-card-copy{min-width:0;flex:1 1 0;overflow:hidden}.cp-event-card time,.cp-event-row time,.cp-external-card time{color:var(--cp-secondary);font-size:16px;font-weight:500;font-style:normal}.cp-event-card time em,.cp-event-row time em,.cp-external-card time em{color:#edb541;font-style:normal;font-weight:600}.cp-event-card h3{display:flex;align-items:center;gap:7px;margin:7px 0 8px;font-size:20px;line-height:1.12;font-weight:600}.cp-event-card h3 svg{padding:4px;border-radius:50%;box-sizing:content-box;color:#ff6ba8;background:rgba(213,23,109,.18)}.cp-event-card p,.cp-event-row p{display:flex;min-width:0;align-items:center;gap:6px;margin:5px 0;color:var(--cp-secondary);font-size:16px;font-weight:500}.cp-event-card p span,.cp-event-row p span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cp-event-card p img,.cp-event-row p img{width:18px;height:18px;flex:0 0 18px;border-radius:50%;object-fit:cover}.cp-event-card button{display:inline-flex;align-items:center;gap:7px;margin-top:12px;padding:7px 10px;border:0;border-radius:8px;color:rgba(255,255,255,.64);background:rgba(255,255,255,.08);font-size:12px;font-weight:650;cursor:pointer;transition:background .16s,color .16s}.cp-event-card button:hover{color:#151515;background:rgba(255,255,255,.64)}.cp-event-cover{display:grid;width:120px;height:120px;flex:0 0 120px;overflow:hidden;place-items:center;border:.5px solid rgba(255,255,255,.04);border-radius:10px;color:var(--cp-secondary);background:rgba(255,255,255,.04)}.cp-event-cover img{width:100%;height:100%;object-fit:cover;transform:scale(1.005)}.cp-event-price{display:inline-flex;align-items:center;padding:4px 7px;border-radius:5px;color:#54c546;background:rgba(60,189,44,.13);font-size:12px;line-height:1;font-weight:600}
