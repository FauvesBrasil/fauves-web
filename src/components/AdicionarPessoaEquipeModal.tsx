...
const niveis = [
  { key: 'socio', label: 'Socio', desc: 'Pode visualizar e gerenciar todo o evento' },
  { key: 'gerente', label: 'Gerente', desc: 'Gerencia tudo menos a conta bancaria para repasses' },
  { key: 'financeiro', label: 'Financeiro', desc: 'Visualiza as informacoes financeiras do evento' },
  { key: 'visualizacao', label: 'Visualizacao', desc: 'Visualiza tudo mas nao pode editar nada' },
  { key: 'checkin', label: 'Check-in', desc: 'Acessa apenas a area de Check-in' },
  { key: 'promoter', label: 'Promoter', desc: 'Ve apenas as vendas/indicacoes atribuidas a ele' },
];

interface ModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (data: { email: string; nivel: string }) => void;
}

export default function AdicionarPessoaEquipeModal({ open, onClose, onAdd }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 relative">
        <button className="absolute top-4 left-4 text-zinc-400 hover:text-zinc-700 text-xl" onClick={onClose}>
          &times;
        </button>
        <div className="flex flex-col items-center mb-6">
          <span className="text-4xl mb-2" role="img" aria-label="colaborador">🧑‍💼</span>
        ...
        />
        <div className="mb-6">
          <label className="block text-sm font-semibold mb-3">Defina um nivel de acesso</label>
