...
const TextLink: React.FC<TextLinkProps> = ({ href, onClick, className = '', children, target, rel }) => {
  const base = 'relative inline-flex items-center font-semibold text-indigo-600 dark:text-white group focus:outline-none transition-transform duration-200 hover:-translate-y-0.5';

  const content = (
    <span className="flex flex-row items-center gap-1 relative z-10">{children}</span>
  );
  ...
  if (href) {
    return (
      <a href={href} target={target} rel={rel} className={`${base} ${className}`}>{content}{underline}</a>
    );
  }

  return (
    <button type="button" onClick={onClick} className={`${base} ${className}`}>{content}{underline}</button>
  );
};
