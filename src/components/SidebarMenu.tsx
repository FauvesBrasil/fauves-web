...
type SidebarMenuProps = {
  ...
  className?: string; // Support for custom styling
};
...
const SidebarMenu: React.FC<SidebarMenuProps> = ({ activeKeyOverride, className }) => {
  ...
  return (
    <div ref={ref} className={`fixed top-0 left-0 flex flex-col justify-between items-center pb-4 bg-[#F8F7FA] dark:bg-[#0b0b0b] border-r border-solid border-gray-100 dark:border-[#1F1F1F] h-screen w-[70px] z-20 max-md:relative max-md:flex-row max-md:gap-5 max-md:p-2.5 max-md:h-auto max-md:w-[60px] max-sm:hidden fauves-left-sidebar ${className || ''}`} data-sidebar="main" data-sidebar-main="true">
      ...
      <div className="flex flex-col justify-end flex-shrink-0 w-full mt-auto">
        <div className="relative flex items-center justify-center mb-6">
          <a
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-indigo-100 dark:hover:bg-[#1F1F1F] transition-colors"
            title="Ajuda"
            onMouseEnter={(e) => showTooltip('Central de Ajuda', e.currentTarget as HTMLElement)}
            onMouseLeave={() => hideTooltip()}
          >
