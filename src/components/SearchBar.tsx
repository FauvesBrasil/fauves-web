...
const SearchBar: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-[727px] h-[33px] border shadow-[0_4px_12.9px_0_rgba(0,0,0,0.05)] flex items-center bg-white px-8 py-0 rounded-[95px] border-solid border-[#E0E0E0] max-md:w-full max-sm:h-10 max-sm:px-5 max-sm:py-0">
      <input
        type="text"
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        placeholder="Pesquisar shows, eventos, espetáculos, cursos..."
        className="flex-1 text-[#C3C3C3] text-sm font-normal max-sm:text-xs bg-transparent border-none outline-none placeholder:text-[#C3C3C3]"
      />
