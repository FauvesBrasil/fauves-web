  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg p-6 w-[380px] max-w-full shadow-lg">
        <h3 className="text-lg font-semibold mb-3">Assinantes</h3>
        <div className="grid grid-cols-4 gap-3">
          {followers.length === 0 && <div className="col-span-4 text-sm text-slate-500">Nenhum assinante encontrado.</div>}
          {followers.map(f => (
            <div key={f.id} className="flex flex-col items-center text-center">
              <Avatar className="w-14 h-14">
                {f.photoUrl ? <AvatarImage src={f.photoUrl} alt={f.name || 'Assinante'} /> : <AvatarFallback>{(f.name || 'U')[0]}</AvatarFallback>}
              </Avatar>
