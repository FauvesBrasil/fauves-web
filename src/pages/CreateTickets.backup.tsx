  ...
  useEffect(() => {
    const loadEventMap = async () => {
      ...
      try {
        ...
        if (!res.ok) return;
        const data = await res.json().catch(() => null);
        const map = data?.map || data?.image || data?.imageUrl || null;
  ...
  useEffect(() => {
    const loadCurrentMap = async () => {
      ...
      try {
        ...
        if (!res.ok) return;
        const data = await res.json().catch(() => null);
        // prefer explicit map column; fallback to banner image if map missing
        const img = data?.map || data?.image || data?.imageUrl || null;
        if (img) setMapPreview(img);
      } catch (e) {
        // ignore
      }
    };
            ...
            <div className="rounded-3xl w-full max-w-[800px] max-sm:max-w-full relative min-h-[600px]">
              <div className="flex flex-col gap-6 max-sm:gap-4 flex-1 mt-[67px] max-sm:mt-4 p-8 max-sm:p-0">
                <div className="flex items-center justify-between mb-4 max-sm:flex-col max-sm:items-stretch max-sm:gap-3">
                  <h1 className="text-[28px] max-sm:text-[24px] font-bold text-indigo-950 dark:text-white">Ingressos</h1>
                  <div className="flex gap-2 items-center max-sm:flex-col max-sm:w-full">
                    ...
                    {/* Mobile: Botões separados quando dropdown está escondido */}
                    <Button
                      variant="outline"
                      className="hidden max-sm:flex w-full h-[44px] items-center justify-center gap-2"
                    ...
                    <Button
                      variant="outline"
                      className="hidden max-sm:flex w-full h-[44px] items-center justify-between gap-2"
                      onClick={() => setMapDrawerOpen(true)}
                    >
                      <span className="flex items-center gap-2">
                        <MapPin className="w-4 h-4" />
                        Adicionar mapa do evento
                      </span>
                      {mapPreview && <CheckIcon size={16} className="text-green-600" />}
                    </Button>
                  </div>
                </div>
                ...
                {ticketsLoading ? (
                  <>
                    <div className="relative w-full">
                      <div className="bg-white dark:bg-[#242424] rounded-2xl border border-[#E5E7EB] dark:border-[#1F1F1F] p-6 max-sm:p-4 mb-2 shadow-sm">
                  ...
                  try {
                    ...
                    if (!res.ok) {
                      ...
                      try {
                        ...
                        // prefer map when present
                        if (updated?.map) setMapPreview(updated.map);
                        else if (updated?.image) setMapPreview(updated.image);
                      } catch (_) { }
