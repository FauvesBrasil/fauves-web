    ...
    try {
      ...
      if (modalType === 'x') {
        payload = {
          ...
          scope: x_scope,
          pixelId: x_pageId,
          config: { positions: x_positions }
        };
      } else if (modalType === 'meta') {
        payload = {
          type: 'meta',
          scope: meta_scope,
          pixelId: meta_pixelId,
          config: { defaultEvents: meta_defaultEvents }
        };
      } else if (modalType === 'ads') {
        payload = {
          type: 'google_ads',
          scope: ads_scope,
          pixelId: ads_conversionId,
          config: { defaultEvents: ads_defaultEvents }
        };
      } else if (modalType === 'ga') {
        payload = {
          type: 'ga',
          scope: ga_scope,
          pixelId: ga_measurementId,
          config: null
        };
        ...
        payload = {
          ...
          scope: img_scope,
          pixelId: 'image-pixel',
          config: { events: img_events }
        };
      }

      const res = await fetchApi(`/api/event/${id}/pixels`, {
        method: 'POST',
  ...
  useEffect(() => {
    ...
    async function loadPixels() {
      ...
      try {
        ...
        if (res.ok && mounted) {
          const data = await res.json();
          setSavedPixels(Array.isArray(data) ? data : []);
        }
      } catch (e) {
        // ignore
      }
      ...
      <OrganizerLayout>
        ...
        <div style={{ marginLeft: totalLeft, transition: 'margin-left 200ms' }} className="flex flex-col pl-8 max-md:pl-4 pr-8 max-md:pr-4 pb-16 max-md:pb-8 relative">
          <div className="mt-24 max-md:mt-[140px] max-w-[800px] mx-auto max-md:max-w-full">
            <div className="max-md:hidden">
              <h1 className="text-3xl font-bold text-indigo-950 dark:text-white mb-3 max-sm:text-2xl">Pixels de rastreamento</h1>
              <p className="text-sm text-gray-600 dark:text-slate-300 mb-6 max-sm:text-xs">O rastreamento de pixels pode ajudar você a medir o impacto de seu marketing, publicidade e análise.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8 max-sm:gap-3">
              {pixelTypes.map(p => (
                <div key={p.key} onClick={() => openModalFor(p.key)} className="cursor-pointer border rounded-xl p-5 flex items-start gap-4 shadow-sm bg-white dark:bg-[#242424] dark:border-[#1F1F1F] hover:shadow-md transition max-sm:p-4 max-sm:gap-3">
                  <div className="flex-shrink-0 w-10 h-10 rounded-md bg-gray-50 dark:bg-[#1F1F1F] flex items-center justify-center text-xl text-indigo-600">
                    {p.key === 'img' ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm dark:text-white max-sm:text-xs">{p.title}</div>
                    <div className="text-xs text-gray-500 mt-1 dark:text-slate-300 max-sm:text-[10px]">+ {p.subtitle}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {modalOpen && modalType === 'x' && (
              <div style={{ zIndex: 1000 }} className="fixed inset-0 flex items-center justify-center p-6 bg-black/40">
                <div className="w-full max-w-2xl bg-white dark:bg-[#242424] rounded-md shadow-xl overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b dark:border-[#1F1F1F]">
                    <div className="flex items-center gap-3">
                      <img src={xLogo} alt="Pixel do X" className="w-6 h-6" />
                      <div className="font-semibold dark:text-white">Adicionar pixel</div>
                    </div>
                    <button onClick={() => setModalOpen(false)} className="text-zinc-500 dark:text-slate-300">✕</button>
                  </div>
                  <div className="p-6 max-h-[60vh] overflow-auto">
                    <div className="mb-4">
                      <div className="text-sm font-medium">Eventos</div>
                      <div className="mt-3 flex gap-4">
                        <label className="inline-flex items-center gap-2"><input type="radio" name="x_scope" checked={x_scope === 'this'} onChange={() => setXScope('this')} /> <span className="dark:text-slate-300">Este evento</span></label>
                        <label className="inline-flex items-center gap-2"><input type="radio" name="x_scope" checked={x_scope === 'all'} onChange={() => setXScope('all')} /> <span className="dark:text-slate-300">Todos os eventos</span></label>
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-2">
                        <input className="w-full border rounded px-3 py-3 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="por exemplo, XXXXX" value={x_pageId} onChange={e => setXPageId(e.target.value)} />
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {x_positions.map((p, idx) => (
                          <div key={idx} className="border rounded p-3 dark:border-[#1F1F1F] dark:bg-[#121212]">
                            <div className="mb-2 text-xs text-zinc-600 dark:text-slate-300">Ação Eventbrite</div>
                            <select className="w-full border rounded px-3 py-2 mb-2 dark:bg-[#121212] dark:border-transparent dark:text-white" value={p.action} onChange={e => updatePosition(idx, { action: e.target.value })}>
                              <option value="event_page">Página do evento</option>
                              <option value="ticket">Compra de ingresso</option>
                            </select>
                            <div className="text-xs text-zinc-600 mb-1 dark:text-slate-300">ID de conversão</div>
                            <input className="w-full border rounded px-3 py-2 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="por exemplo, XXXX" value={p.conversionId} onChange={e => updatePosition(idx, { conversionId: e.target.value })} />
                  ...
                  <div className="flex items-center justify-end gap-4 px-6 py-4 border-t">
                    <button className="px-4 py-2 rounded bg-zinc-50 text-sm dark:bg-[#1F1F1F] dark:text-slate-300" onClick={() => setModalOpen(false)}>Cancelar</button>
                ...
                <div className="w-full max-w-2xl bg-white dark:bg-[#242424] rounded-md shadow-xl overflow-hidden">
                  ...
                  <div className="p-6 max-h-[60vh] overflow-auto">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-2">
                        <input className="w-full border rounded px-3 py-3 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="por exemplo, 1234567890" value={meta_pixelId} onChange={e => setMetaPixelId(e.target.value)} />
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {meta_defaultEvents.map((r, idx) => (
                          <div key={idx} className="border rounded p-3 dark:border-[#1F1F1F] dark:bg-[#121212]">
                            <div className="mb-2 text-xs text-zinc-600 dark:text-slate-300">Ação Eventbrite</div>
                            <select className="w-full border rounded px-3 py-2 mb-2 dark:bg-[#121212] dark:border-transparent dark:text-white" value={r.action} onChange={e => updateMetaDefaultEvent(idx, { action: e.target.value })}>
                              <option value="event_page">Página do evento</option>
                              <option value="ticket">Compra de ingresso</option>
                            </select>
                            <div className="text-xs text-zinc-600 mb-1 dark:text-slate-300">Evento padrão</div>
                            <input className="w-full border rounded px-3 py-2 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="Nome do evento padrão" value={r.eventName} onChange={e => updateMetaDefaultEvent(idx, { eventName: e.target.value })} />
                            <div className="mt-2 text-right">
                ...
                <div className="w-full max-w-2xl bg-white dark:bg-[#242424] rounded-md shadow-xl overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b dark:border-[#1F1F1F]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-semibold dark:text-white">Adicionar pixel Google Ads</div>
                    </div>
                    <button onClick={() => setModalOpen(false)} className="text-zinc-500 dark:text-slate-300">✕</button>
                  </div>
                  <div className="p-6 max-h-[60vh] overflow-auto">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-2">
                        <input className="w-full border rounded px-3 py-3 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="ID de conversão" value={ads_conversionId} onChange={e => setAdsConversionId(e.target.value)} />
                      </div>
                    </div>
                    <div className="mb-4">
                      <div className="flex items-center justify-between">
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {ads_defaultEvents.map((r, idx) => (
                          <div key={idx} className="border rounded p-3 dark:border-[#1F1F1F] dark:bg-[#121212]">
                            <div className="mb-2 text-xs text-zinc-600 dark:text-slate-300">Ação Eventbrite</div>
                            <select className="w-full border rounded px-3 py-2 mb-2 dark:bg-[#121212] dark:border-transparent dark:text-white" value={r.action} onChange={e => updateAdsDefaultEvent(idx, { action: e.target.value })}>
                              <option value="event_page">Página do evento</option>
                              <option value="ticket">Compra de ingresso</option>
                              <option value="checkout">Checkout</option>
                            </select>
                            <div className="text-xs text-zinc-600 mb-1 dark:text-slate-300">ID de conversão</div>
                            <input className="w-full border rounded px-3 py-2 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="por exemplo, XXXX" value={r.conversionId} onChange={e => updateAdsDefaultEvent(idx, { conversionId: e.target.value })} />
                            <div className="mt-2 text-right">
                ...
                <div className="w-full max-w-2xl bg-white dark:bg-[#242424] rounded-md shadow-xl overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b dark:border-[#1F1F1F]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-semibold dark:text-white">Adicionar pixel Google Analytics</div>
                  ...
                  <div className="p-6 max-h-[60vh] overflow-auto">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-2">
                        <input className="w-full border rounded px-3 py-3 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="G-XXXXXXXXXX" value={ga_measurementId} onChange={e => setGaMeasurementId(e.target.value)} />
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center justify-end gap-4 px-6 py-4 border-t">
                ...
                <div className="w-full max-w-2xl bg-white dark:bg-[#242424] rounded-md shadow-xl overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b dark:border-[#1F1F1F]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-semibold dark:text-white">Pixel de imagem simples</div>
                  ...
                  <div className="p-6 max-h-[60vh] overflow-auto">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="mt-4 space-y-3">
                        {img_events.length === 0 && <div className="text-sm text-zinc-500 dark:text-slate-300">Nenhum evento adicionado</div>}
                        {img_events.map((r, idx) => (
                          <div key={idx} className="border rounded p-3 dark:border-[#1F1F1F] dark:bg-[#121212]">
                            <div className="mb-2 text-xs text-zinc-600 dark:text-slate-300">Ação Eventbrite</div>
                            <select className="w-full border rounded px-3 py-2 mb-2 dark:bg-[#121212] dark:border-transparent dark:text-white" value={r.action} onChange={e => updateImgEvent(idx, { action: e.target.value })}>
                              <option value="event_page">Página do evento</option>
                              <option value="ticket">Compra de ingresso</option>
                              <option value="checkout">Checkout</option>
                            </select>
                            <div className="text-xs text-zinc-600 mb-1 dark:text-slate-300">URL do pixel</div>
                            <input className="w-full border rounded px-3 py-2 dark:bg-[#121212] dark:border-transparent dark:text-white" placeholder="https://example.com/pixel.gif" value={r.url} onChange={e => updateImgEvent(idx, { url: e.target.value })} />
                            <div className="mt-2 text-right">
            ...
            <div className="bg-white border border-zinc-200 rounded-xl dark:bg-[#242424] dark:border-[#1F1F1F]">
              <div className="px-6 py-3 border-b text-sm text-zinc-600 grid grid-cols-4 dark:text-slate-300 dark:border-[#1F1F1F] max-sm:px-4 max-sm:text-xs">
              ...
              {savedPixels.length === 0 ? (
                <div className="p-8 text-center text-sm text-zinc-500 dark:text-slate-300 max-sm:p-4 max-sm:text-xs">Nenhum pixel de rastreamento configurado</div>
              ) : (
                <div className="divide-y divide-zinc-100 dark:divide-[#1F1F1F]">
                  {savedPixels.map((pixel) => (
                    <div key={pixel.id} className="px-6 py-4 grid grid-cols-4 items-center text-sm max-sm:px-4 max-sm:text-xs">
                      <div className="flex items-center gap-2">
                        {pixel.type === 'meta' && <img src={metaLogo} alt="Meta" className="w-5 h-5" />}
                        {pixel.type === 'google_ads' && <img src={googleAdsLogo} alt="Google Ads" className="w-5 h-5" />}
                        {pixel.type === 'ga' && <img src={gaLogo} alt="GA" className="w-5 h-5" />}
                        {pixel.type === 'x' && <img src={xLogo} alt="X" className="w-5 h-5" />}
                        {pixel.type === 'img' && <span className="text-zinc-500">📍</span>}
                        <span className="dark:text-white capitalize">{pixel.type}</span>
                      </div>
                      <div className="text-zinc-600 dark:text-slate-300 truncate" title={pixel.pixelId}>{pixel.pixelId}</div>
                      <div>
                        <span className={`px-2 py-1 rounded-full text-xs ${pixel.scope === 'all' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300'}`}>
                          {pixel.scope === 'all' ? 'Todos' : 'Este evento'}
                        </span>
                      </div>
                      <div>
                        <button
            ...
            <div className="mt-6 max-sm:mt-4">
              <a className="inline-flex items-center gap-2 rounded-md bg-zinc-50 px-4 py-2 text-sm text-zinc-700 border dark:bg-[#1F1F1F] dark:text-slate-300 dark:border-[#1F1F1F] max-sm:text-xs max-sm:px-3 max-sm:py-2" href="/ajuda/organizador" target="_blank" rel="noopener noreferrer">
                <svg className="w-4 h-4 text-zinc-600 dark:text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /></svg>
